...
from datasets.corruptions_datasets import create_cifarc_dataset, create_imagenetc_dataset, ImageNet100CStreamDataset
...
def get_transform(dataset_name: str, adaptation: str, preprocess: Union[transforms.Compose, None], use_clip: bool, n_views: int = 64):
    ...
    if use_clip:
        ...
        elif dataset_name in ["imagenet_c", "imagenet100_c", "ccc"]:
...
def get_test_loader(setting: str, adaptation: str, dataset_name: str, preprocess: Union[transforms.Compose, None],
    ...
    if dataset_name == "imagenet100_c":
        if not stream_csv:
            raise ValueError(
                "ImageNet100-C evaluation now expects a sequential stream CSV. "
                "Please set CORRUPTION.STREAM_CSV to the CSV produced by build_sequential_stream.ipynb."
            )

        test_dataset = ImageNet100CStreamDataset(
            csv_path=stream_csv,
            data_dir=data_dir,
            transform=transform,
        ...
        logger.info(f"Loaded ImageNet100-C sequential stream from: {stream_csv}")
        logger.info(f"Number of stream samples: {len(test_dataset)}")
