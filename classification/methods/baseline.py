...
class Baseline(TTAMethod):
    ...
    def loss_calculation(self, x, y):
        ...
        random_idx = random.sample(range(len(imgs_test)), self.oracle_num)
        sorted_idx = random_idx
        
        ids = [i for i in range(len(imgs_test)) if i not in sorted_idx].to(self.device) # remaining_indices
        
        loss_un =  entropys[ids].mean() # unsupervised loss
        loss_ce = F.cross_entropy(outputs[sorted_idx], y[sorted_idx]) # supervised loss
