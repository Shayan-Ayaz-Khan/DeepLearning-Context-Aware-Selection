...
from datasets.corruptions_datasets import get_imagenet_subset_indices
...
def subset_linear_classifier(classifier: nn.Linear, class_indices):
    subset = nn.Linear(classifier.in_features, len(class_indices), bias=classifier.bias is not None)
    with torch.no_grad():
        subset.weight.copy_(classifier.weight[class_indices])
        if classifier.bias is not None:
            subset.bias.copy_(classifier.bias[class_indices])
    return subset


def subset_classifier_head(model: nn.Module, class_indices):
    model_core = model.model if isinstance(model, nn.Sequential) and hasattr(model, "model") else model

    if hasattr(model_core, "fc") and isinstance(model_core.fc, nn.Linear):
        model_core.fc = subset_linear_classifier(model_core.fc, class_indices)
    elif hasattr(model_core, "heads") and hasattr(model_core.heads, "head") and isinstance(model_core.heads.head, nn.Linear):
        model_core.heads.head = subset_linear_classifier(model_core.heads.head, class_indices)
    elif hasattr(model_core, "classifier"):
        classifier = model_core.classifier
        if isinstance(classifier, nn.Linear):
            model_core.classifier = subset_linear_classifier(classifier, class_indices)
        elif isinstance(classifier, nn.Sequential) and isinstance(classifier[-1], nn.Linear):
            classifier[-1] = subset_linear_classifier(classifier[-1], class_indices)
        else:
            raise ValueError(f"Unsupported classifier structure for ImageNet100-C: {type(classifier)}")
    else:
        raise ValueError(f"Unable to find a linear classifier head for ImageNet100-C in model: {model_core.__class__.__name__}")

    return model


def maybe_subset_imagenet100_model(cfg, model: nn.Module, num_classes: int):
    if cfg.CORRUPTION.DATASET != "imagenet100_c":
        return model

    data_dir = complete_data_dir_path(cfg.DATA_DIR, cfg.CORRUPTION.DATASET)
    class_indices = list(get_imagenet_subset_indices(data_dir, expected_num_classes=num_classes))
    return subset_classifier_head(model, class_indices)
...
def get_torchvision_model(model_name: str, weight_version: str = "IMAGENET1K_V1"):
    ...
    preprocess = transforms.Compose([
                                    # transforms.Resize(transform.resize_size, interpolation=transform.interpolation),
...
def get_model(cfg, num_classes: int, device: Union[str, torch.device]):
    ...
    if cfg.MODEL.EDGE_CLOUD:
        if cfg.MODEL.CLOUD_ARCH == "vit_b_16" or cfg.MODEL.CLOUD_ARCH == 'vit_l_16' or cfg.MODEL.CLOUD_ARCH == 'vit_h_14':
        ...
        cloud_model = maybe_subset_imagenet100_model(cfg, cloud_model, num_classes)
        edge_model = maybe_subset_imagenet100_model(cfg, edge_model, num_classes)
    ...
    if cfg.MODEL.USE_CLIP:
        ...
        base_model = maybe_subset_imagenet100_model(cfg, base_model, num_classes)
